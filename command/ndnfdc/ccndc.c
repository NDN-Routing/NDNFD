          ...
          const char *cmd_orig)
{
    ...
    printf("cmd_uri:\t%s\ncmd_proto:\t%s\ncmd_host:\t%s\ncmd_port:\t%s\ncmd_flags:\t%s\ncmd_mcastttl:\t%s\ncmd_mcastif:\t%s\n", cmd_uri, cmd_proto, cmd_host, cmd_port, cmd_flags, cmd_mcastttl, cmd_mcastif);
                        ...
                        int freshness)
{
    ...
    if (strcasecmp(cmd_proto, "ether") != 0) {
		...
		goto ExitOnError;
    	    ...
    	    off_address = entry->store->length;
    	    res = ccn_charbuf_append(entry->store, cmd_host, strlen(cmd_host)+1);
    	    if (res != 0) {
    	    	ccndc_warn(__LINE__, "Cannot append to charbuf");
		goto ExitOnError;
    	    }
